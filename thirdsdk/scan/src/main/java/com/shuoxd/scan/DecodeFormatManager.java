...
public final class DecodeFormatManager {
    ...
    static {
        ...
        FORMATS_FOR_MODE.put(com.growatt.scan.Intents.Scan.ONE_D_MODE, ONE_D_FORMATS);
        FORMATS_FOR_MODE.put(com.growatt.scan.Intents.Scan.PRODUCT_MODE, PRODUCT_FORMATS);
        FORMATS_FOR_MODE.put(com.growatt.scan.Intents.Scan.QR_CODE_MODE, QR_CODE_FORMATS);
        FORMATS_FOR_MODE.put(com.growatt.scan.Intents.Scan.DATA_MATRIX_MODE, DATA_MATRIX_FORMATS);
        FORMATS_FOR_MODE.put(com.growatt.scan.Intents.Scan.AZTEC_MODE, AZTEC_FORMATS);
        FORMATS_FOR_MODE.put(com.growatt.scan.Intents.Scan.PDF417_MODE, PDF417_FORMATS);
    }
    ...
    static Set<BarcodeFormat> parseDecodeFormats(Intent intent) {
        ...
        CharSequence scanFormatsString = intent.getStringExtra(com.growatt.scan.Intents.Scan.FORMATS);
        if (scanFormatsString != null) {
            scanFormats = Arrays.asList(COMMA_PATTERN.split(scanFormatsString));
        }
        return parseDecodeFormats(scanFormats, intent.getStringExtra(com.growatt.scan.Intents.Scan.MODE));
    }

    static Set<BarcodeFormat> parseDecodeFormats(Uri inputUri) {
        List<String> formats = inputUri.getQueryParameters(com.growatt.scan.Intents.Scan.FORMATS);
        if (formats != null && formats.size() == 1 && formats.get(0) != null) {
            formats = Arrays.asList(COMMA_PATTERN.split(formats.get(0)));
        }
        return parseDecodeFormats(formats, inputUri.getQueryParameter(com.growatt.scan.Intents.Scan.MODE));
    }
